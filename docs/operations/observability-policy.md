...
- `HTTP request completed` logs must stay payload-free: status, route template, duration, `request_id`, `trace_id`, and `span_id` are allowed; user prompts and raw request bodies are not.
